...
function FollowersModal() {
  ...
  useEffect(() => {
    if (open) {
      //to fetch the users whom the user is Following
            ...
            <button className="outline-none float-right" onClick={() => setOpen(false)}>
              ...
              <div as="h3" className="flex items-center space-x-4  text-2xl ">
                <div className=" flex items-center justify-center h-14 w-14 rounded-full bg-red-100">
                  <UserGroupIcon className="h-8 w-8 text-red-600 mt-[2px]" />
                </div>
                <span>Following</span>
              </div>
              <hr />
              <div className="overflow-y-scroll space-y-3 max-h-96 scrollbar-hide">
                {users?.map((user) => (
